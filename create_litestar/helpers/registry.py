...
from create_litestar.helpers.constants import MANIFEST_URL, REQUEST_TIMEOUT, TARBALL_URL
...
def get(url: str) -> bytes:
    if not url.startswith("https://"):
        raise CreateLitestarError(f"refusing to fetch a non-HTTPS url: {url}")
    try:
...
def fetch_templates() -> tuple[Template, ...]:
    ...
    if not templates:
        raise CreateLitestarError(f"the template registry at {MANIFEST_URL} is empty")
    return tuple(
...
def find_template(templates: Sequence[Template], name: str) -> Template:
    ...
    raise CreateLitestarError(
        f"unknown template {name!r}.{hint} Run 'create-litestar --list' to see every template"
    )
